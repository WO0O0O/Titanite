...
- **Vercel Route Caching:** Added `export const revalidate = 900` to [route.ts (market)](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/app/api/market/route.ts), [route.ts (intel)](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/app/api/intel/route.ts), and [route.ts (watchlist)](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/app/api/watchlist/route.ts). Vercel now caches each route's response for 15 minutes at the edge — all concurrent users share a single upstream call per window regardless of user count.
- **Client Poll Interval:** Changed `refetchInterval` from `60_000` ms (60 seconds) to `900_000` ms (15 minutes) in [useMarketData.ts](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/hooks/useMarketData.ts), [useIntelFeed.ts](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/hooks/useIntelFeed.ts), and [useHoldings.ts](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/hooks/useHoldings.ts) to align with the server-side cache window and eliminate pointless wakeups against a cached endpoint.
- **European Ticker Symbol Map:** Added `TICKER_YAHOO_MAP` in [route.ts (watchlist)](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/app/api/watchlist/route.ts) mapping display tickers to correct Yahoo Finance query symbols and their native currency codes. Covers: SIVE→SIVE.ST (SEK), IQE→IQE.L (GBp), LPK/LPKF→LPK.F (EUR), TPEG→TPE.F (EUR), SOITEC→SOI.PA (EUR), XFAB→XFAB.PA (EUR), GAPW→6GW.F (EUR), ACCON→4AC.F (EUR), SEYE→S9E.F (EUR), EOS→EOS.AX (AUD), SEE→SEE.AX (AUD), KAORI→8996.TW (TWD), NCI→4092.T (JPY).
- **Native Currency Display:** Added `currency` field to [watchlist.ts](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/types/watchlist.ts) type. Updated [WatchlistTable.tsx](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/components/dashboard/WatchlistTable.tsx) price formatter to display prices in their native currency (e.g. `SEK 32.50`, `EUR 16.20`, `GBP 0.52`). GBp (pence) is automatically divided by 100 and shown as GBP. Failed fetches now show `—` instead of a misleading `$100.00` placeholder. Price column widened to `min-w-[110px]` to accommodate currency prefixes. Column headers updated to `PRICE (NATIVE)` and `MKT CAP (USD)`.
- **Mock Data Correction:** Updated mock fallback prices in [useHoldings.ts](file:///Users/danwooster/1.%20DEV/titanite-technologies/src/hooks/useHoldings.ts) with accurate native-currency prices and correct `currency` fields for all known tickers.
...
**Rationale:** Updated Sivers Semiconductors ($SIVE) to reflect its share price decline (from SEK 78.80 to SEK 63.35) and corresponding USD/SEK exchange rate shift (9.73). Corrected a scorecard addition calculation error to restore its true mathematical score of **12/13 (Tier 1)**. Integrated newly surfaced OSINT detailing the indirect supply chain exposure to NVIDIA's TFLN evaluation through Jabil's high-volume module packaging.
...
- **Scorer Report Refinement:** Updated [SIVE-RESEARCH-REPORT.md](file:///Users/danwooster/1.%20DEV/titanite-technologies/Titanite-Research/notes/SMALLCAP-AI-INFRA/photonics/SIVE-RESEARCH-REPORT.md) Gate Check metrics (USD market cap down to $2.027 billion, sales multiple down to 86.2x) and Section 5 Cluster Scaling Return Matrix (asymmetric return multiple increased to 7.10x return). Corrected scorecard total, verdict, and synthesis rating to 12/13. Integrated Jabil 1.6T LRO transceiver module and NVIDIA TFLN supply chain connections in Section 2, Section 8, and Section 15. Added recent AGM and price pullback details in Section 13.
- **Central Index Integration:** Updated Sivers' description and corrected score to 12/13 in [TABLE.md](file:///Users/danwooster/1.%20DEV/titanite-technologies/Titanite-Research/docs/TABLE.md).
